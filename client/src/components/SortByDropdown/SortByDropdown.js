...
export default function SortByDropdown({value, ...props}) {
    ...
    const variant = id !== null ? 'primary' : "secondary";

    return <>
        <Dropdown items={sortItems} value={id} onSelect={onSelect} variant={variant}>Sort By</Dropdown>
        {id !== null && <Button className="ms-1" variant="secondary" onClick={() => props.onSelect(null)}>X</Button>}
    </>
}